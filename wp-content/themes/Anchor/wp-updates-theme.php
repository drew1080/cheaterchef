...
if( !class_exists('WPUpdatesAnchorUpdater') ) {
    class WPUpdatesAnchorUpdater {
		...
		function __construct( $api_url,  $theme_slug ) {
			...
			add_filter( 'pre_set_site_transient_update_themes', array(&$this, 'Anchor_check_for_update_') );
			add_action( 'after_theme_row_'.$theme_name, array(&$this, 'Anchor_child_theme_row') );
			...
			if(!strstr($_SERVER['REQUEST_URI'],'plugin-install.php') && !strstr($_SERVER['REQUEST_URI'],'update.php'))
			{
				...
				add_action( 'load-update-core.php', array(&$this,'Anchor_clear_update_transient') );
				add_action( 'load-themes.php', array(&$this, 'Anchor_clear_update_transient') );
				if(!strstr($_SERVER['REQUEST_URI'],'/network/')){
					add_action( 'admin_notices', array(&$this, 'Anchor_child_update_nag') );
				}
				delete_transient( $theme_name.'-update' );
		...
		function Anchor_child_update_nag($transient) {
			...
			{	
				echo '<div id="update-nag">';
				 $new_version = version_compare($theme_version, $remote_version, '<') ? __('There is a new version of Anchor available.', T_DOMAIN) .' <a class="thickbox" title="Anchor Forms" href="http://templatic.com/members/mydownloads/Anchor/theme/change_log.txt">'. sprintf(__('View version %s Details', T_DOMAIN), $remote_version) . '</a>. ' : '';		  			
			          $theme_name = basename(get_stylesheet_directory_uri());
					...
					$ajax_url=site_url('/wp-admin/admin.php?page=Anchor_tmpl_theme_update');
					$download= wp_nonce_url( self_admin_url('update.php?action=upgrade-theme&theme=').$theme_name, 'upgrade-theme_' . $theme_name);
					echo '</tr><tr class="plugin-update-tr"><td colspan="3" class="plugin-update"><div class="update-message">' . $new_version . __( 'or <a href="'.$ajax_url.'" title="Anchor Update">update now</a>.', T_DOMAIN) .'</div></td>';
				echo '</div>';
			}
		...
		function Anchor_check_for_update_( $transient ) {
			global $theme_response,$wp_version;
			...
			$request_string = $this->Anchor_prepare_request( 'templatic_theme_update', $request_args );
		 ...
		 */
		function Anchor_child_theme_row()
		{
			global $theme_response,$wp_version;			
		...
		function Anchor_prepare_request( $action, $args ) {
			global $wp_version;
