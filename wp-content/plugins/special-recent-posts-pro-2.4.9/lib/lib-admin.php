...
function srpcheck_phpver_error() {
	...
	$error = new WP_Error('broke', __("<strong>Special Recent Posts PRO Edition Error!</strong> You're running an old version of PHP. In order for this plugin to work, you must enable your server with PHP support version 5.0.0+. Please contact your hosting/housing company support, and check how to enable it.</a>", SRP_TRANSLATION_ID));
...
function srpcheck_gd_error() {
	...
	// Setting up new Error.
	$error = new WP_Error('broke', __("<strong>Special Recent Posts PRO Edition Error!</strong> GD libraries are not supported by your server. Please contact your hosting/housing company support, and check how to enable it. Without these libraries, thumbnails can't be properly resized and displayed.", SRP_TRANSLATION_ID));
	if (is_wp_error($error)) {
	   echo "<div id=\"message\" class=\"error\"><p>" . $error->get_error_message() . "</p></div>";
	}
}
...
function srpcheck_cache_exists_error() {
	...
	// Setting up new Error.
	$error = new WP_Error('broke', __("<strong>Special Recent Posts PRO Edition Warning!</strong> The Cache folder does not exist!. In order to use caching functionality you have to manually create a folder names 'cache' under the special-recent-posts/ folder.", SRP_TRANSLATION_ID));
...
function srpcheck_cache_writable_error() {
	...
	// Setting up new Error.
	$error = new WP_Error('broke', __("<strong>Special Recent Posts PRO Edition Warning!</strong> The Cache folder is not writable. In order to use caching functionality you have to set the correct writing permissions on special-recent-posts/cache/ folder. E.G: 0755 or 0775", SRP_TRANSLATION_ID));
	if (is_wp_error($error)) echo "<div id=\"message\" class=\"error\"><p>" . $error->get_error_message() . "</p></div>";
}
...
function srp_admin_setup() {
	...
	// Adding SubMenu Page.
	$srp_settings_page = add_submenu_page('options-general.php', __('Special Recent Posts PRO Edition - Settings Page', 'Special Recent Posts PRO Edition - Settings Page'), __('Special Recent Posts PRO Edition', 'Special Recent Posts PRO Edition'), 'administrator', __FILE__, 'srp_admin_menu_options');
...
?>
	...
	<div class="wrap">
		<div id="srp-admin-container">
			...
			<h2 class="srp_admin_headertitle"><?php _e('Special Recent Posts PRO Edition (version ' . SRP_PLUGIN_VERSION . ') - Settings Page', SRP_TRANSLATION_ID); ?></h2>
			<div class="srp_option_header_l1">
			<?php _e('<strong>Welcome to the Special Recent Posts PRO Edition Admin Panel.</strong><br /> In this page you can configure the main settings for the Special Recent Posts PRO Edition plugin. Keep in mind that these are basic options. 
			Special options apply for each widget instance, shortcode or PHP code to ensure an high level of customization. Go to Widget Page and drag the Special Recent Posts PRO Edition widget to see additional options available.', SRP_TRANSLATION_ID); ?>
			<h3>The plugin CSS has now been moved into a single included file under special-recent-posts-pro-2.4.8/css/css-front.css</h3>
			</div>
						...
						<div id="srp-admin-leftcontent">
							...
							<dl>
								...
								<dd>
									...
									If you are experiencing problems with Special Recent Posts PRO Edition and other plugins, you might want to disable this option.', SRP_TRANSLATION_ID); ?>
								</dd>
