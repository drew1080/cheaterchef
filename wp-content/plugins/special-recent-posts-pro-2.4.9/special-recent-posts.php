...
Description: Special Recent Posts PRO Edition is a very powerful plugin/widget which displays your latest posts with thumbnails. <strong>The perfect solution for online magazines or simple blogs, it comes with more than 60+ customization options available.</strong> <strong>To get started: 1)</strong>  Click the "Activate" link to the left of this description. <strong>2)</strong> Go to the Special Recent Post settings page and configure the plugin. <strong>3)</strong> Go to the Widgets page and drag the 'Special Recent Posts PRO Edition' widget onto your sidebar and configure its settings. <strong>4)</strong> If you wish to use PHP code or shortcodes for your pages, simply copy and paste the code provided under the panel "Code Generator" in the widget options. Or, alternatively, please refer to the manual provided in the plugin folder. You can also check the readme.txt file for further details. <strong>5)</strong> Enjoy.
Version: 2.4.9
...
define('SRP_PLUGIN_VERSION'      , '2.4.9');                                     // Defining plugin version.
...
function special_recent_posts($args = array()) {
	...
	if (SpecialRecentPostsPro::visualizationCheck($args, 'phpcall')) {
...
function srp_shortcode($atts) {
	...
	// Checking Visualization filter.
	if (SpecialRecentPostsPro::visualizationCheck($srp_default_widget_values, 'shortcode')) {
...
function srp_plugin_action_links($links, $file) {
    ...
    if ($file == $this_plugin) {
        ...
        $settings_link = '<a href="' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=special-recent-posts-pro-' . SRP_PLUGIN_VERSION . '/lib/lib-admin.php">Settings</a>';
...
register_activation_hook(__FILE__     , array('SpecialRecentPostsPro', 'install_plugin'));   // Registering plugin activation hook.
register_uninstall_hook( __FILE__     , array('SpecialRecentPostsPro', 'uninstall_plugin')); // Registering plugin uninstall hook.
